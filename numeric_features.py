    ...
    for b in range( bins ):
        if bin_data[b].shape[0] != 0:
            ...
            for train_index, test_index in kf.split( bin_data[b] ):
                ...
                # print ( helpers[b].iloc[train_index][numeric] )
                ...
                # reg.fit( helpers[b][numeric], helpers[b]['SalePrice'] )
                reg.fit( X_train, y_train )

                # pred = reg.predict( helpers[b][numeric] )
                pred = reg.predict( X_test )

                # error = abs( pred - helpers[b]['SalePrice'] )
